...
```bash
# Step 1 - start
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Define the problem: need to optimize slow API" -n 3

# Step 2 - continue
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Identify cause: N+1 query in user list" -n 3 -s SESSION_ID

# Step 3 - conclude
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Solution: add eager loading for user relations" -n 3 -s SESSION_ID
```

### Revision

```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Actually, the issue is not N+1 but missing index" -n 5 \
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Alternative: use caching instead of query optimization" -n 5 \
...
```bash
# Create assumption
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Assuming Redis is available for caching" -n 5 \
  --assumptions '[{"id":"A1","text":"Redis is available","critical":true}]'

# Depend on assumption
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Design cache layer using Redis" -n 5 \
  --depends-on '["A1"]' -s SESSION_ID

# Invalidate assumption
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Redis not available, need alternative" -n 5 \
...
```bash
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Assuming the API works" -n 3 \
...
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Redis will work" -n 3 \
  --assumptions '[{"id":"A1","text":"Redis latency < 5ms"}]'
# Returns session_id: "abc-123"

# Session 2: Reference from session 1
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Building on previous assumption" -n 3 \
...
```bash
# Capture session_id
SESSION=$(uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Start analysis" -n 5 | jq -r '.session_id')

# Continue
uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Continue analysis" -n 5 -s "$SESSION"

# Check if more thoughts needed
CONTINUE=$(uv run ${CLAUDE_PLUGIN_ROOT}/skills/ultrathink/scripts/ultrathink.py -t "Step" -n 5 -s "$SESSION" | jq -r '.next_thought_needed')
```
